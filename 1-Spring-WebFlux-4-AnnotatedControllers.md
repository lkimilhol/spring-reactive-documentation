...
Spring WebFlux는 @Controller 및 @RestController 구성
요소가 어노테이션을 사용하여 요청 매핑, 요청 입력, 예외 처리 등을 표현하는 어노테이션 기반 프로그래밍 모델을 제공합니다. 어노테이션이 달린 컨트롤러는 유연한 메서드 시그네쳐를 가지고 있으며 기본 클래스를 확장하거나 특정 인터페이스를 구현할 필요가 없습니다.
...
@RequestMapping 핸들러 메소드는 유연한 시그네쳐를 가지고 있으며 지원되는 다양한 컨트롤러 메소드 아규먼트 및 반환 값 중에서 선택할 수 있습니다.
